...
<html>
 ...
 <table>
    ...
    <?php
     ...
     while($stmt->fetch()) {
     echo '<tr><td>'.$BID.'</td><td>'.$FullName.'</td><td>'.$LastName.'</td><td>'.$Address.'</td><td>'.$Email.'</td><td>'.$Date.'</td><td>'.$Time.'</td><td>'.$City.'</td><td>'.$Package.'</td><td><a href="mail.php?pkg='.$Package.'&amp;Mail='.$Email.'" >Send mail</a></td></tr>';
     }
     ?>
       </tbody>
     </table>;
